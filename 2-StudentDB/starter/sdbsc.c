...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    // Check if it is a valid location to place
    off_t offset = id * STUDENT_RECORD_SIZE;
    if (lseek(fd, offset, SEEK_SET) == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    ...
    // Check if location is currently used
    student_t existing_student = {0};
    if (read(fd, &existing_student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        if (existing_student.id != DELETED_STUDENT_ID) {
